          ...
          <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-50">
              ...
              >
                  ...
                  {me?.isHost && (
                    <button
                      onClick={restartGame}
                      className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 uppercase italic"
                    >
                      <Crown size={16}/> თამაშის გაგრძელება
                    </button>
                  )}
...
function ElectionPanel({ room, playerId, vote, continueElection }: { room: GameRoom; playerId: string; vote: (v: 'Ja' | 'Nein') => void; continueElection: () => void }) {
  ...
  const voters = room.players.filter(p => p.alive && p.id !== room.currentPresidentId && p.id !== room.currentChancellorCandidateId);
    ...
    <div className="bg-[#1a1a1a] border border-[#333] rounded-2xl p-4 md:p-6 space-y-4">
      ...
      {electionResult ? (
        ...
        <div className="space-y-3">
          <div className="bg-[#111] border border-[#333] rounded-xl p-4 text-center text-[#888] font-bold">
            {room.currentPresidentId === playerId ? 'პრეზიდენტი არჩევნებში ხმას არ აძლევს.' : 'კანცლერობის კანდიდატი არჩევნებში ხმას არ აძლევს.'}
          </div>
          {voteRows}
        </div>
      ) : hasVoted ? (
